...
class CoralsForm
{
    ...
    protected $selectTypes = ['boolean', 'select', 'select2', 'select2-tree'];
    ...
    public function input($key, $label = '', $required = false, $value = null, $attributes = [], $type = 'text')
    {
        ...
        if (in_array($type, $this->selectTypes)) {
            ...
            if ($type == 'file') {
                $image = \Arr::get($attributes, 'with_preview', true) ? '<img  src="#" alt="" class="preview hidden" width="100"/>' : '';
    ...
    public function customFields($model, $fieldClass = 'col-md-4', $customFields = [], $propColumn = 'properties')
    {
        ...
        foreach ($customFields as $field) {
            ...
            $value = $model->exists ? $model->getProperty($name, null, null, $propColumn) : Arr::get($fields, 'default_value');
    ...
    public function handleCustomFieldInput($field, $value = null)
    {
        ...
        switch ($fieldType = Arr::get($field, 'type')) {
            ...
            case 'checkbox':
                ...
                break;
            case 'radio':
                $input = $this->{$fieldType}($name, $fieldLabel, $isFieldRequired, $options, $value, $fieldCustomAttributes);
                break;
            case 'select':
                $input = $this->{$fieldType}($name, $fieldLabel, $options, $isFieldRequired, $value, $fieldCustomAttributes, 'select2');
                break;
            case 'multi_values':
                ...
                $attributes = array_merge(['class' => 'select2-normal tags', 'multiple' => true], $fieldCustomAttributes);
    ...
    protected function renderCustomFieldsContent($fields)
    {
        ...
        foreach ($fields as $fieldConfig) {
            $renderedFields .= sprintf("<div class='%s'>%s</div>", $fieldConfig['grid_class'], $fieldConfig['content']);
        }
