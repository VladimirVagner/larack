    ...
    function appendDetails(&$formattedResponse, $detailsArray)
    {
        if (is_array($detailsArray)) {
            foreach ($detailsArray as $key => $value) {
                ...
                $keyTitle = str_replace('_', ' ', \Illuminate\Support\Str::title($key));
                ...
                if (is_array($value)) {
                    ...
                    $formattedResponse .= "<td><b style='word-break: break-all;'>{$value}</b></td>";
                }
            }
        }
    }
...
if (!function_exists('get_media_url')) {
    function get_media_url($media, $download = false)
    {
