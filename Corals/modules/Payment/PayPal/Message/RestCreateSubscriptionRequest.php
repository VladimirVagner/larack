...
class RestCreateSubscriptionRequest extends AbstractRestRequest
{
    ...
    public function getData()
    {
        ...
        function send_to_zoho($order_id) {

            $is_status_set = get_post_meta( $order_id , '_status_set', true );
            if($is_status_set =="yes"){
                $order = new WC_Order($order_id);
                $order_status  = $order->get_status();
                update_post_meta( $order_id, 'status_date_'.$order_status, date('Y-m-d H:i:s')  );
                if(in_array($order_status, ['completed','cancelled','returned'])){
                    update_post_meta( $order_id, 'completed_date_'., date('Y-m-d H:i:s')  );
                }
                if(class_exists('vxc_zoho')){
                    $vxc_zoho = new vxc_zoho();
                    $vxc_zoho->push($order_id,'update');
                }
            }
